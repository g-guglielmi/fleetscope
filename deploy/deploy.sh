...
docker run -d --name fs-app --restart unless-stopped \
  ...
  -e FS_PUBLIC_URL="${FS_PUBLIC_URL:-}" \
  -e FS_SECRETS_KEY="${FS_SECRETS_KEY:-}" \
  -e FS_SIGNING_PUBKEY="${FS_SIGNING_PUBKEY:-}" \
  -e FS_AGENT_CHECKIN_SECONDS="${FS_AGENT_CHECKIN_SECONDS:-120}" \
  -e FS_DEV_MODE="${FS_DEV_MODE:-false}" \
